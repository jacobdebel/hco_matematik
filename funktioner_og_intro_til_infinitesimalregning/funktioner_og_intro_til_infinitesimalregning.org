...
#+begin_export html
<iframe width="560" height="315" src="https://www.youtube.com/embed/rNo5xfjxj6c?si=3I1hxWzrxz3f-T_3" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>
#+end_export
...
3. Bestem den tilbagelagte strækning i det fulde interval 0 til 65 s vha integralregning. (Del op i mindre funktioner for hver bevægelsestype.)
#+reveal_html: </div>
...
- Altså $f(x)=a\cdot x^2 + b \cdot x +c =0$
...
** Brændpunkt og konstruktion
#+reveal_html: <div style="font-size: 60%;">
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
- Vælg en /ledelinje/ \(y=k\).
- Punktet \(Q\) er parablens /brændpunkt/.
- Ethvert punkt \(P\) på parablen har samme til \(Q\) som den lodrette afstand til ledelinjen.
- Altså \(\left| QP \right|=\left| PR \right|\)
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->
[[file:img/braendpunkt_parabel_konstruktion.png]]
#+reveal_html: </div>
#+reveal_html: </div>
#+reveal_html: </div>
...
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
Skriv følgende i inputfeltet i =geogebra=:
#+begin_example
Q=(0,5) 
L=(-1,0) 
k:y=y(L) 
...
g:Vinkelret(A,k)
B:skæring(g,k)
c:Cirkel(Q,afstand(A,B))
P:Skæring(c,f)
r:Linjestykke(Q,P_1)
m:Linjestykke(A,B)
#+end_example
- Højreklik på ~r~, vælg egenskaber, vælg navn & værdi
- Højreklik på ~m~, vælg egenskaber, vælg navn & værdi
- Højreklik på ~P_1~, vælg tænd spor.
- Højreklik på ~P_2~, vælg tænd spor.
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->


- Træk nu i punktet ~A~ for at afsætte en masse punkter.
- Flyt ~Q~ for at flytte brændpunktet.
- Flyt ~L~ for at flytte ledelinjen.
- Få forskrift ved at skrive: =parabel(Q,k)=

#+attr_reveal: :frag (appear)
- Hvorfor hedder det mon et brændpunkt?
- Tangenter, punkter og vinkler "to the rescue!"
#+reveal_html: </div>
#+reveal_html: </div>
...
- Ellers er de her:
  \begin{align*}
  f'(x) &= \frac{d}{dx}\left(a x^2 + b x +c \right) = a \cdot 2 x^{2-1} +b \iff \\ f'(x)&= 2 a x + b \\
  F(x) &= \int a x^2 + b x +c \,dx = a \cdot \frac{x^{2+1}}{2+1} + b \cdot \frac{x^{1+1}}{1+1} + c \cdot x + k \iff \\
  F(x) &= \frac{a}{3} x^3 + \frac{b}{2} x^2 + c x + k
  \end{align*}
...
hvilken indvirkning har så $k$, $a$ og $b$ på grafen?
#+reveal_html: </div>
...
#+reveal_html: <div style="font-size: 60%;">
#+begin_quote
Hvis den uafhængige variabel x i en potensfunktion:

\[f(x)=bx^a \text{, hvor } x>0 \text{ og } b \ne 0\]

multipliceres med en koeffient \(k\), hvor \(k>0\), vil det medføre, at den afhængige variabel \(f(x)\) multipliceres med \(k^a\):

\[f(k x) = k^a \cdot f(x) \]
#+end_quote

#+attr_reveal: :frag (appear)
Bevis: \(f(kx) = b \cdot \left( k \cdot x \right)^a = b \cdot k^a\cdot x^a = k^a\cdot b x^a= k^a\cdot f(x)\).
#+reveal_html: </div>
...
\[S_B(v_0)=\frac{1}{2a}\cdot v_0^2\]

"Hvis du fordobler din hastighed, 4-dobler du bremselængden."

\[S_B(2\cdot v_0)=2^2 \cdot S_B(v_0) = 4 \cdot S_B(v_0)\]
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->
*Stofskifte/Kleibers lov*

Basal Metabolic Rate

\[BMR = b M^{\frac{3}{4}}\]

En elefant vejer ca 10 000 gange mere end en mus.

BMR for en elefant er derfor /kun/ \(10000^{\frac{3}{4}}=1000\) gange større end for en mus.
#+reveal_html: </div>
#+reveal_html: </div>
...
#+reveal_html: <div> <!-- left -->
#+reveal_html: <div style="font-size: 60%;">
- Læs og forstå [[https://matb-htx.systime.dk/?id=1663#c15745][Procentvis vækst]].

#+reveal_html: </div>
#+reveal_html: <div style="overflow: auto; height: 500px;"> <!-- scroll -->
[[file:img/procentvis_vaekst_forklaring.png]]

[[file:img/procentvis_vaekst_eksempel_1.png]]

[[file:img/procentvis_vaekst_eksempel_2.png]]
#+reveal_html: </div>
...
#+attr_reveal: :frag (appear)
- Når \(x\) vokser med en procentsats betyder det, at \(x\) skal *multipliceres* med en faktor.
- Hvis \(x\) vokser med *procentsatsen* \(r_x\), så skal \(x\) *multipliceres* med \(k=1+r_x\).
- En procentvis tilvækst beregnes ved først at bestemme den absolutte tilvækst og derefter dividere den med startværdien.
- Den relative tilvækst for \(y\)-værdien er derfor \(r_y = \frac{\Delta y}{y}= \frac{f(kx)-f(x)}{f(x)}\)
- For \(f(x)=b x^a\) bliver det da \(r_y =\frac{f((1+r_x)x)-f(x)}{f(x)}= \frac{(1+r_x)^a \cdot f(x) - f(x)}{f(x)} \)
- \(r_y =\frac{f(x) \cdot \left( (1+r_x)^a - 1 \right)}{f(x)}= (1+r_x)^a-1\)
#+reveal_html: </div>
#+reveal_html: </div>
#+reveal_html: </div>
...
** Opgaver
:PROPERTIES:
:ID:       56bb0b86-7673-4394-a825-c7a9d39f7941
:END:
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
#+attr_html: :width 600px
[[file:img/2026-09-17_16-34-37_screenshot.png]]
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->
#+attr_html: :width 600px
[[file:img/2026-09-17_16-35-09_screenshot.png]]
#+reveal_html: </div>
#+reveal_html: </div>
...
#+begin_quote
Hvis to punkter \(P_1=(x_1,y_1)\) og \(P_2=(x_2,y_2)\) er beliggende på grafen for en potensfunktion af typen \(f(x)=bx^a\), hvor \(x > 0\) og \(b \ne 0\), så er:

\[a = \frac{{\log \left( {\frac{{{y_2}}}{{{y_1}}}} \right)}}{{\log \left( {\frac{{{x_2}}}{{{x_1}}}} \right)}}= \frac{\log(y_2)-\log(y_1)}{\log(x_2) - \log(x_1)}\]

og:

\[b = \frac{{{y_1}}}{{{x_1}^a}} \text{ eller } b = \frac{{{y_2}}}{{{x_2}^a}}\]
#+end_quote
...
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
[[file:img/potensfunktioner_eksempel_1.png]]
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->
[[file:img/potensfunktioner_eksempel_2.png]]
#+reveal_html: </div>
#+reveal_html: </div>

** Opgaver
:PROPERTIES:
:ID:       eae043ef-767e-4663-997d-f6a91e6f9ca6
:END:
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
#+attr_html: :width 600px
[[file:img/2026-09-17_16-36-06_screenshot.png]]
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->

#+DOWNLOADED: screenshot @ 2026-09-17 16:36:28
#+attr_html: :width 600px
#+attr_latex: :width 14cm
[[file:img/2026-09-17_16-36-28_screenshot.png]]
#+reveal_html: </div>
#+reveal_html: </div>
...
#+attr_reveal: :frag (appear)
Skrivemåder:
\[f(g(x)) = f \circ g (x) \quad \text{og} \quad g(f(x)) = g \circ f (x) \]

#+reveal_html: </div>
** Dm for sammensatte funktioner
#+reveal_html: <div style="font-size: 60%;">
#+begin_quote
*Værdimængden* for den *indre* funktion skal være en del af *definitionsmængden* af den *ydre* funktion.
#+end_quote

#+attr_reveal: :frag (appear)
- Man kan nemlig få problemer.

#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
#+attr_reveal: :frag (appear)
- eks: \(f(x)=2x-5 \quad g(x)= \sqrt{x}\)
- Her er \(Dm(f)=]-\infty , \infty [\), \(Vm(f) = ]-\infty , \infty[\)
- Her er \(Dm(g)=[0 , \infty [\), \(Vm(f) = [0 , \infty[\)
...
#+attr_reveal: :frag (appear)
- Dur fint med \(x=7\)
- \(f(7)=2 \cdot 7 - 5 = 9 \quad , \, g(9) = \sqrt{9}=3\)
- \(g(f(7)) = \sqrt{2 \cdot 7 - 9}=3\)
- Dur *ikke* med \(x=2\)
- \(f(2) = 2 \cdot 2 -5 = -1 \quad , \, g(-1) = \sqrt{-1}\)
- Man kan ikke tage kvadratroden af et negativt tal, hvis man kun har de reelle tal at gøre godt med.
#+reveal_html: </div>
#+reveal_html: </div>
...
** Opløsning af forskrift
#+reveal_html: <div style="font-size: 60%;">
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
*Eksempel 1*

\(h(x)=\left( 4x+7 \right)^5\)

\(f(x)= 4x+7 \quad \text{indre funktion}\)
\(g(x)= x^5 \quad \text{ydre funktion}\)
#+reveal_html: </div>
...
\(f(x)= 3x^2-7x \quad \text{indre funktion}\)
\(g(x)= \sqrt{x} \quad \text{ydre funktion}\)
#+reveal_html: </div>
#+reveal_html: </div>

#+reveal_html: </div>

** Opgave - find indre og ydre funktioner
#+reveal_html: <div style="font-size: 60%;">
Identificér de indre og ydre funktioner i de følgende tilfælde:

#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
\begin{align*}
f_1(x) &= (4x-1)^3 \\
f_2(x) &= 3\cdot (x^2+5)^4 \\
f_3(x) &= \sqrt{2x+7}
\end{align*}
#+reveal_html: </div>
...
#+reveal_html: <div> <!-- right -->
\begin{align*}
f_4(x) &= \frac{1}{x^2-3x+2} \\
f_5(x) &= \sqrt[3]{5x^3-2x} \\
f_6(x) &= \left((2x-1)^2+3\right)^5
\end{align*}
#+reveal_html: </div>
#+reveal_html: </div>
#+reveal_html: </div>
...
#+begin_quote
For den sammensatte funktion $h(x)=g(f(x))$ gælder, at
$$\boxed{h'(x) = g'(f(x))\cdot f'(x) = \frac{d g(f(x))}{d f(x)}\cdot \frac{d f(x)}{dx}}$$
#+end_quote
...
** Eksempel
#+reveal_html: <div style="font-size: 60%;">
$$h(x)=\left( 2\cdot x^3+5x \right)^4$$
#+attr_reveal: :frag (appear)
- Den ydre funktion er $g(x)=x^4$
- Den indre funktion er $f(x)=2\cdot x^3 + 5x$
- Først finder man $g'(x)=4 x^3$, men på x's plads skriver man den indre funktion, altså
- $g'(f(x))=4 \cdot \left( 2\cdot x^3+5x \right)^3$
- Så differentierer man den indre funktion på alm. vis
- $f'(x) = 2 \cdot 3 \cdot x^2+5 = 6x^2+5$
- men dette skal lige ganges på det første, altså
- $h'(x) = g'(f(x))\cdot f'(x) = 4 \cdot \left( 2\cdot x^3+5x \right)^3 \cdot \left( 6x^2+5 \right)$
#+reveal_html: </div>

** Et andet eksempel
#+reveal_html: <div style="font-size: 50%;">
Vi gør det lige en gang til (Den er lidt svær).
$$h(x) = \sqrt{\left( 4x^3 + 1 -2x \right)^2-6x^2}$$
...
** Tre funktioner
:PROPERTIES:
:ID:       235ed34e-401b-49fa-96ea-10c49b7c8a26
:END:
#+reveal_html: <div style="font-size: 60%;">
...
#+DOWNLOADED: screenshot @ 2026-09-24 10:39:01
#+attr_html: :width 600px
#+attr_latex: :width 14cm
[[file:img/2026-09-24_10-39-01_screenshot.png]]
...
** Gaffelfunktion
:PROPERTIES:
:ID:       3c19a7aa-5bb5-441e-ab78-1f57c0534d2e
:END:
#+reveal_html: <div style="font-size: 50%;">
#+reveal_html: <div style="display: grid; grid-template-columns: 50% auto;">
#+reveal_html: <div> <!-- left -->
Én funktion med tre forskellige forskrifter i hvert sit interval.

\[f(x) = \begin{cases} x+1 & \text{for } 0 \leq x \leq 4 \\ - \frac{1}{4}x^2+3x-3 & \text{for }  4 < x \leq 10 \\ -2x+22 & \text{for } 10 < x \leq 11\end{cases}\]

#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->

#+DOWNLOADED: screenshot @ 2026-09-24 10:44:53
#+attr_html: :width 600px
#+attr_latex: :width 14cm
[[file:img/2026-09-24_10-44-53_screenshot.png]]

#+reveal_html: </div>
#+reveal_html: </div>
Geogebrakommando: =f(x)=hvis(0≤x≤4,x+1,4<x≤10,-1/4x^2+3x-3,10<x≤11,-2x+22)=
#+reveal_html: </div>

** Gaffel
:PROPERTIES:
:ID:       4cf2c55d-4ffa-414f-9a22-cd2ddc73e5a5
:END:
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->
Gemini's bedste bud
[[file:img/gaffelfunktion.jpeg]]
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->
Min forestilling

#+DOWNLOADED: screenshot @ 2026-09-24 11:14:12
#+attr_html: :width 600px
#+attr_latex: :width 14cm
[[file:img/2026-09-24_11-14-12_screenshot.png]]
...
** Eksempel/opgave
:PROPERTIES:
:ID:       f12307e3-0893-464e-9a9b-0e439abaafa0
:END:
#+reveal_html: <div style="font-size: 60%;">
#+reveal_html: <div style="display: grid; grid-template-columns: 40% auto;">
#+reveal_html: <div> <!-- left -->

#+DOWNLOADED: screenshot @ 2026-09-24 11:15:48
#+attr_html: :width 600px
#+attr_latex: :width 14cm
[[file:img/2026-09-24_11-15-48_screenshot.png]]
#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->
1. Opstil de tre forskrifter vha beregninger.
2. Opskriv en samlet gaffelfunktion.
3. Hvornår har bilen en fart på 40 km/t?


#+attr_reveal: :frag (appear)
- Hint: Brug topunktsformlen for rette linjer. Husk også at bestemme b.

#+attr_reveal: :frag (appear)  
Svar
#+attr_reveal: :frag (appear)  
1. \(g(x)=5.33x \,,\, h(x)=80 \,,\, k(x)=-8x+320\)
2. \(f(x)=\begin{cases}5.33 x & \text{for } 0 \leq x \leq 15 \\ 80 & \text{for} 15 < x \leq 30 \\ -8x+320 & \text{for } 30 < x \leq 40 \end{cases}\)
3. Efter 7.5 s og 35 s.
#+reveal_html: </div>
#+reveal_html: </div>
...
** Flere opgaver
:PROPERTIES:
:ID:       965a9443-bbf1-4c44-81bb-8ab5dc8fe2a9
:END:
#+reveal_html: <div style="display: grid; grid-template-columns: auto auto;">
#+reveal_html: <div> <!-- left -->

#+DOWNLOADED: screenshot @ 2026-09-24 11:31:10
#+attr_html: :width 600px
#+attr_latex: :width 14cm
[[file:img/2026-09-24_11-31-10_screenshot.png]]

#+reveal_html: </div>

#+reveal_html: <div> <!-- right -->

#+DOWNLOADED: screenshot @ 2026-09-24 11:31:44
#+attr_html: :width 600px
#+attr_latex: :width 14cm
[[file:img/2026-09-24_11-31-44_screenshot.png]]
...
* Projekt om eksponential- og logaritmefunktioner

* Trigonometriske funktioner

* Projekt tidevand og diger

* Funktioner med polære koordinater
